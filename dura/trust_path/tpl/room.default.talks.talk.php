				<?php foreach ( $dura['room']['talks'] as $talk ) : ?>
					<?php if ( !$talk['uid'] ) : ?>
						<div class="talk system" id="<?php e($talk['id']) ?>" <?php e($talk['time'] <= $dura['input']['last_talk_time'] ? ' dura-show="1"' : ''); ?> >
							<?php e($talk['message']) ?>
						</div>
					<?php else: ?>
						<dl class="talk icon_<?php e($talk['icon']) ?> <?php if ( $talk['uid'] == $dura['room']['host'] ) :?> dura-ishost <?php endif; ?>" data-uid="<?php e($talk['uid']) ?>" id="<?php e($talk['id']) ?>" <?php e($talk['time'] <= $dura['input']['last_talk_time'] ? ' dura-show="1"' : ''); ?> >
